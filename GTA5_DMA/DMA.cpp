...
bool DMA::Initialize()
{
	LPCSTR args[] = { "", "-device", "FPGA", "-printf", "-v" };

	PLC_CONFIG_ERRORINFO pErrorInfo = nullptr;
	vmh = VMMDLL_InitializeEx(5, args, &pErrorInfo);

	if (!vmh)
	{
		std::println("VMMDLL_Initialize failed (FPGA device).");
		if (pErrorInfo && pErrorInfo->cwszUserText)
		{
			std::wstring msg(pErrorInfo->wszUserText, pErrorInfo->cwszUserText);
			std::println("LeechCore: {}", std::string(msg.begin(), msg.end()));
		}
		else
		{
			std::println("Check: FPGA plugged into this PC, FTD3XX.dll next to exe, FTDI drivers installed.");
		}

		if (pErrorInfo)
			VMMDLL_MemFree(pErrorInfo);
		return 0;
	}
